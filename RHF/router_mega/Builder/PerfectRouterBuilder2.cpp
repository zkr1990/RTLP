...
//---------------------------------------------------------------------------
bool TPerfectRouterBuilder2::Build(int *pn, uint32_t *hash,int nHash,int nBucket,int nSlot, int s0, int s1)
// bool TPerfectRouterBuilder2::Build(int *pn, uint32_t *hash,int nHash,int nBucket,int nSlot, int sl0, int sl1, int s0, int s1)
{
	...
	for(int i=0;i<nSlot;i++)
	{
		...
		for(int d=1;d<maxD;d++)
		{
			...
			{
				// uint32_t x_key = sortedHash[startPos+j] >> 8;
				// int f = ( ((x_key * (uint32_t)d) >> 8) << shiftBucket >> shiftBucket);
				uint32_t x_key = sortedHash[startPos+j];
				// int f = ( ((x_key >> s0) ^ ((x_key << s1)  + (uint32_t)d) ^ x_key) << shiftBucket >> shiftBucket);
				int f = ( (((x_key >> s0) + (uint32_t)d) ^ ((x_key << s1) ^ x_key)) << shiftBucket >> shiftBucket);
				// int f = ( (((x_key >> nSlotWidth) + (uint32_t)d) ^ (x_key << bucketWidth) ^ x_key) << shiftBucket >> shiftBucket);
